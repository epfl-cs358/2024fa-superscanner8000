...
if __name__ == "__main__":
    ...
    posx = Entry(root)
    posx.insert(0, "0")
    posx.grid(row=3, column=1)
    posy = Entry(root)
    posy.insert(0, "0")
    posy.grid(row=3, column=1)
    Button(root, text="Goto", command=lambda: car.arm.goto(int(posy.get()), int(posy.get()))).grid(row=3, column=0)
    Button(root, text="Stop Arm", command=car.arm.stop).grid(row=3, column=2)
